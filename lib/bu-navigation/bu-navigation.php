		...
		foreach ($pages as $page)
		{
			...
			else if ($page->post_parent != 0)
			{
				...
				if( $parent ) {
					...
					while (isset($parent->post_parent) && $parent->post_parent != 0)
					{
						$parent = bu_navigation_pull_page($parent->post_parent, $pages);

						// If a post parent is in the trash it won't be in $pages
...
/**
 * Fetch page tree in jstree json_data format
 *
 * Relocated from the navigation plugin (bu-navman.php) to remove dependency on plugin.
 * @deprecated 1.5.1
 * @deprecated Use the BU_Navigation_Tree_View class
 * @see BU_Navigation_Tree_View class
 *
 * @param int $id post ID to start from
 * @param array $pages_by_parent associative array of posts data, keyed on parent ID
 * @return array requested posts in jstree json_data format
 */
function bu_navman_get_children($id, $pages_by_parent) {
	$pages = array();
...
/**
 * Adds exclude and ACL restriction data to pages requested during bu_navigation_get_pages
 *
 * This is the default filter added while getting pages during RPC requests to rpc/get-pages.php.
 * Relocated from the navigation plugin (bu-navman.php) to remove dependency on plugin.
 */
